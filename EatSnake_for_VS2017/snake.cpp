...
void GameStart()
{
	mciSendString("open 游戏BGM.mp3 alias 游戏BGM", 0, 0, 0);	//打开BGM
	mciSendString("play 游戏BGM repeat", 0, 0, 0);			//播放BGM
	SnakeInit();	//初始化蛇
	while (1)
	{
		cleardevice();	//刷新帧
...
/*
	这段代码可直接放进SnakeMove里判定，减少主循环中执行的函数。
//	判断蛇是否撞墙
void IfHitWall()
{
	if (snake.xy[0].x > 490 || snake.xy[0].x < 100 || snake.xy[0].y > 500 || snake.xy[0].y < 100)
	{
		//mciSendString("pause 游戏BGM", 0, 0, 0);	//暂停BGM
		mciSendString("open boom.mp3 from 0", 0, 0, 0);	//撞墙音效
		mciSendString("play boom.mp3", 0, 0, 0);
		GameOver();		//撞墙时,游戏结束
	}
...
void GameOver()
{
	...
	else outtextxy(530, 200, endWall);
	outtextxy(530, 250, tips);
	outtextxy(530, 225, end);
	while (_getch() != INPUT_PAUSE);
	cleardevice();
	mciSendString("pause 游戏BGM", 0, 0, 0);	//暂停BGM
	mciSendString("open gameover.mp3 from 0", 0, 0, 0);	//游戏结束BGM
	mciSendString("play gameover.mp3 repeat", 0, 0, 0);
	char choose;
	char End[] = "GAME OVER";
	char go_on[] = "按y开始下一局";
	char go_exit[] = "按其他任意键退出";
	settextstyle(60, 0, "华文琥珀");
	settextcolor(COLORREF RGB(134, 235, 246));
	outtextxy(260, 125, End);
	settextstyle(25, 0, "宋体");
	outtextxy(330, 200, go_on);
	outtextxy(320, 250, go_exit);
	settextcolor(COLORREF RGB(144, 236, 194));
	choose = _getch();
	switch (choose)
	{
	...
	case 'Y':
		...
		mciSendString("play 游戏BGM from 0", 0, 0, 0);	//播放BGM
		GameStart(); break;
	case 'n':
	case 'N':
		exit(0); break;
